...
int maix_read_reg(uint8_t addr, uint8_t reg, uint8_t *data)
{
    ...
    *data = recv_data;
    if(res != 0)
        LOGE(TAG, "i2c read reg error!, recv_data: %d", recv_data);
    return res;
}
//static
int maix_i2c_init()
{
    //set io mux
    fpioa_set_function(CONFIG_PIN_NUM_ES8374_I2C_SCL, FUNC_I2C0_SCLK + ES8374_I2C_DEVICE * 2);
    fpioa_set_function(CONFIG_PIN_NUM_ES8374_I2C_SDA, FUNC_I2C0_SDA + ES8374_I2C_DEVICE * 2);
...
int es8374_set_mic_gain(es_mic_gain_t gain)
{
    ...
    if(gain > MIC_GAIN_MIN && gain < MIC_GAIN_24DB)
    {
        int gain_n = 0;
        gain_n = (int)gain / 3;
        uint8_t reg = (gain | (gain_n << 4));
        res = es8374_write_reg(0x22, reg); //MIC PGA
    } else
    {
        res = -1;
        LOGW(TAG, "invalid microphone gain!");
    }
...
static int es8374_set_adc_dac_volume(int mode, int volume, int dot)
{
    ...
    {
        LOGW(TAG, "Warning: volume < -96! or > 0!");
        if(volume < -96)
        {
            volume = -96;
        } else
...
static int es8374_init_reg(es8374_mode_t ms_mode, es_i2s_fmt_t fmt, es_i2s_clock_t cfg, es_dac_output_t out_channel, es_adc_input_t in_channel)
{
    ...
    res |= es8374_write_reg(0x26, 0x4E); // alc set
...
int es8374_ctrl_state(es8374_mode_t mode, es8374_ctrl_t ctrl_state)
{
    ...
    switch(mode)
    {
        ...
        default:
            es_mode_t = ES_MODULE_DAC;
            LOGE(TAG, "Codec mode not support, default is decode mode\r");
            break;
    }
    ...
    } else
    {
        ...
        LOGI(TAG, "start default is decode mode:%d\r", es_mode_t);
    }
    return res;
}
