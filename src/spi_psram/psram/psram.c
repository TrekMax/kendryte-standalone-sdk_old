...
psram_status_t (*psram_page_program_fun)(uint32_t addr, uint8_t *data_buf, uint32_t length);
psram_status_t (*psram_read_fun)(uint32_t addr, uint8_t *data_buf, uint32_t length);
static psram_status_t psram_stand_read_data(uint32_t addr, uint8_t *data_buf, uint32_t length);
static psram_status_t psram_quad_read_data(uint32_t addr, uint8_t *data_buf, uint32_t length);

static psram_status_t psram_page_program(uint32_t addr, uint8_t *data_buf, uint32_t length);
static psram_status_t psram_quad_page_program(uint32_t addr, uint8_t *data_buf, uint32_t length);
...
static psram_status_t psram_receive_data_enhanced(uint32_t *cmd_buff, uint8_t cmd_len, uint8_t *rx_buff, uint32_t rx_len)
{
    spi_receive_data_multiple(spi_bus_no, spi_chip_select, cmd_buff, cmd_len, rx_buff, rx_len);
    return PSRAM_OK;
}

static psram_status_t psram_send_data_enhanced(uint32_t *cmd_buff, uint8_t cmd_len, uint8_t *tx_buff, uint32_t tx_len)
{
...
psram_status_t psram_init(uint8_t spi_index, uint8_t spi_ss)
{
    ...
    psram_page_program_fun = psram_page_program;
    psram_read_fun = psram_stand_read_data;
    return PSRAM_OK;
}
...
psram_status_t psram_read_id(uint8_t *manuf_id, uint8_t *device_id)
{
    ...
    psram_receive_data(cmd, 4, data, 2);
    *manuf_id = data[0];
    *device_id = data[1];
    return PSRAM_OK;
}

static psram_status_t psram_write_enable(void)
{
    uint8_t cmd[1] = {WRITE_ENABLE};

    psram_send_data(cmd, 1, 0, 0);
    return PSRAM_OK;
}
...
psram_status_t psram_read_status_reg1(uint8_t *reg_data)
{
    ...
    psram_receive_data(cmd, 1, data, 1);
    *reg_data = data[0];
    return PSRAM_OK;
}
...
psram_status_t psram_enable_quad_mode(void)
{
    ...
    psram_read_status_reg2(&reg_data);
    if (!(reg_data & REG2_QUAL_MASK))
    {
        reg_data |= REG2_QUAL_MASK;
        psram_write_status_reg(0x00, reg_data);
    }
    psram_page_program_fun = psram_quad_page_program;
    psram_read_fun = psram_quad_read_data;
    return PSRAM_OK;
}
...
static psram_status_t psram_page_program(uint32_t addr, uint8_t *data_buf, uint32_t length)
{
    ...
    cmd[3] = (uint8_t)(addr);
    psram_write_enable();
    psram_send_data(cmd, 4, data_buf, length);
...
static psram_status_t psram_quad_page_program(uint32_t addr, uint8_t *data_buf, uint32_t length)
{
    ...
    psram_send_data_enhanced(cmd, 2, data_buf, length);
    while (psram_is_busy() == PSRAM_BUSY)
        ;
    return PSRAM_OK;
}

static psram_status_t psram_sector_program(uint32_t addr, uint8_t *data_buf)
{
    uint8_t index = 0;

    for (index = 0; index < psram_FLASH_PAGE_NUM_PER_SECTOR; index++)
    {
        psram_page_program_fun(addr, data_buf, psram_FLASH_PAGE_SIZE);
        addr += psram_FLASH_PAGE_SIZE;
        data_buf += psram_FLASH_PAGE_SIZE;
    }
    return PSRAM_OK;
}
...
psram_status_t psram_write_data(uint32_t addr, uint8_t *data_buf, uint32_t length)
{
    ...
    uint8_t *pread = NULL;
    uint8_t *pwrite = NULL;
    uint8_t swap_buf[psram_FLASH_SECTOR_SIZE] = {0};

    while (length)
    {
        sector_addr = addr & (~(psram_FLASH_SECTOR_SIZE - 1));
        sector_offset = addr & (psram_FLASH_SECTOR_SIZE - 1);
        sector_remain = psram_FLASH_SECTOR_SIZE - sector_offset;
        write_len = ((length < sector_remain) ? length : sector_remain);
        psram_read_fun(sector_addr, swap_buf, psram_FLASH_SECTOR_SIZE);
        ...
        for (index = 0; index < write_len; index++)
        {
            ...
            {
                psram_sector_erase(sector_addr);
                while (psram_is_busy() == PSRAM_BUSY)
                    ;
                break;
            }
        ...
        if (write_len == psram_FLASH_SECTOR_SIZE)
        {
            psram_sector_program(sector_addr, data_buf);
        }
        else
        {
            pread = swap_buf + sector_offset;
            ...
            for (index = 0; index < write_len; index++)
                *pread++ = *pwrite++;
            psram_sector_program(sector_addr, swap_buf);
        }
        length -= write_len;
        addr += write_len;
        data_buf += write_len;
...
psram_status_t psram_write_data_direct(uint32_t addr, uint8_t *data_buf, uint32_t length)
{
    ...
    while (length)
    {
        page_remain = psram_FLASH_PAGE_SIZE - (addr & (psram_FLASH_PAGE_SIZE - 1));
        write_len = ((length < page_remain) ? length : page_remain);
        psram_page_program_fun(addr, data_buf, write_len);
        length -= write_len;
        addr += write_len;
        data_buf += write_len;
    }
...
static psram_status_t _psram_read_data(uint32_t addr, uint8_t *data_buf, uint32_t length, psram_read_t mode)
{
    uint32_t cmd[2] = {0};
    ...
    switch (mode)
    {
        case PSRAM_STANDARD:
            ...
            *(((uint8_t *)cmd) + 3) = (uint8_t)(addr >> 0);
            psram_receive_data((uint8_t *)cmd, 4, data_buf, length);
            break;
        case PSRAM_STANDARD_FAST:
            *(((uint8_t *)cmd) + 0) = FAST_READ;
            *(((uint8_t *)cmd) + 1) = (uint8_t)(addr >> 16);
            *(((uint8_t *)cmd) + 2) = (uint8_t)(addr >> 8);
            *(((uint8_t *)cmd) + 3) = (uint8_t)(addr >> 0);
            *(((uint8_t *)cmd) + 4) = 0xFF;
            psram_receive_data((uint8_t *)cmd, 5, data_buf, length);
            break;
        case PSRAM_DUAL:
            ...
            spi_init_non_standard(spi_bus_no, 8/*instrction length*/, 24/*address length*/, 8/*wait cycles*/,
                                  SPI_AITM_STANDARD/*spi address trans mode*/);
            psram_receive_data_enhanced(cmd, 2, data_buf, length);
            break;
        case PSRAM_DUAL_FAST:
...
psram_status_t psram_read_data(uint32_t addr, uint8_t *data_buf, uint32_t length, psram_read_t mode)
{
    ...
    while (length)
    {
        ...
        _psram_read_data(addr, data_buf, len, mode);
        addr += len;
        data_buf += len;
        length -= len;
    }
...
static psram_status_t psram_stand_read_data(uint32_t addr, uint8_t *data_buf, uint32_t length)
{
    return psram_read_data(addr, data_buf, length, PSRAM_STANDARD_FAST);
}

static psram_status_t psram_quad_read_data(uint32_t addr, uint8_t *data_buf, uint32_t length)
{
    return psram_read_data(addr, data_buf, length, PSRAM_QUAD_FAST);
}
