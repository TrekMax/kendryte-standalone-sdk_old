...
{
    spi_set_clk_rate(SD_SPI_INDEX, 10000000);
}

static void sd_lowlevel_init(uint8_t spi_index)
{
    gpiohs_set_drive_mode(SD_SPI_CS_PIN, GPIO_DM_OUTPUT);
    spi_set_clk_rate(SD_SPI_INDEX, 200000);     /*set clk rate*/
}

static void sd_write_data(uint8_t *data_buff, uint32_t length)
{
    spi_init(SD_SPI_INDEX, SPI_WORK_MODE_0, SPI_FF_STANDARD, 8, 0);
    spi_send_data_standard(SD_SPI_INDEX, SPI_CHIP_SELECT_3, NULL, 0, data_buff, length);
}
...
static void sd_read_data(uint8_t *data_buff, uint32_t length)
{
    ...
    spi_init(SD_SPI_INDEX, SPI_WORK_MODE_0, SPI_FF_STANDARD, 8, 0);
    spi_receive_data_standard(SD_SPI_INDEX, SPI_CHIP_SELECT_3, NULL, 0, data_buff, length);
...
{
    spi_init(SD_SPI_INDEX, SPI_WORK_MODE_0, SPI_FF_STANDARD, 8, 0);
    spi_send_data_standard_dma(DMAC_CHANNEL0, SD_SPI_INDEX, SPI_CHIP_SELECT_3, NULL, 0, (uint8_t *)(data_buff), 128 * 4);
}

static void sd_read_data_dma(uint8_t *data_buff)
{
    spi_init(SD_SPI_INDEX, SPI_WORK_MODE_0, SPI_FF_STANDARD, 8, 0);
    spi_receive_data_standard_dma(-1, DMAC_CHANNEL0, SD_SPI_INDEX, SPI_CHIP_SELECT_3,NULL, 0, data_buff,128 * 4);
}
